...
OpenCamInterop is an experimental interoperability layer being incubated inside IPCamLapse. It gives camera, NVR, and automation projects a small common event envelope while keeping each source format's unavoidable differences visible.

The useful long-term direction is an EventLab workflow: capture a problematic event sequence, sanitize it, replay it deterministically in CI, and turn a hardware-specific quirk into an executable compatibility test. This first slice establishes the event contracts and the privacy-safe fixture seam. It does not rename or replace IPCamLapse.

## What is implemented

- A packable .NET 8 `OpenCamInterop` library using the official CloudEvents C# JSON formatter
...
2. Put it beneath `OpenCamInterop/fixtures/v1/{adapter}`.
3. Add an assertion to `IPCamLapse.Tests/OpenCamInteropAdapterTests.cs` that proves the new behavior or diagnostic.
4. Explain the interoperability gap and the information deliberately removed during sanitization.
5. Run the focused tests and the full repository checks from `CONTRIBUTING.md`.
...
Before extracting OpenCamInterop into a separately named repository, the project should earn that split with at least one external consumer, three meaningful sanitized traces spanning two vendors, and a stable capture/inspect/replay/verify CLI contract. Until then, keeping it in this repository avoids an empty public shell while still offering real contribution seams.
