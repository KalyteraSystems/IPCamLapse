...
The container stores its Data Protection key ring in the persistent data volume with owner-only directory permissions. The key ring is not protected by a separate encryption key, so treat the volume and its backups as secrets. Do not copy, publish, or commit key files. Restore the key ring together with profile and session JSON; deleting it makes saved passwords unrecoverable.
